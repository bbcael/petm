% what this shows:
    % fluctuations are laplace distributed
    % laplace fluctuations can arise from characteristic-rate relaxation 
    % petm is anomalous
...
for i = 1:(length(t)-1); % average d18O and d13C within time bins
    ...
    % SENSITIVITY TEST #2 – use median rather than mean [n.b. doesn't change conclusions]
...
% SENSITIVITY TEST #5 – try fit for e.g. different ranges of "os" for state-dependence of exponential parameter 
%   [n.b. weak state-dependence, but doesn't change conclusions]
[eo,uo] = mle(xo,'pdf',@(x,b)1/(2*b)*exp(-abs(x)/b),'Start',[.1]);
% SENSITIVITY TEST #6 – try different probability distributions 
%   [n.b. Laplace is best fit with fewest free parameters and simplest mechanistic explanation, doesn't change conclusions]
ceo = heaviside(xo)-.5.*sign(xo).*exp(-abs(xo)/eo(1)); y = linspace(0,1,length(xo)); vo = max(ceo-y) + max(y-ceo);
[ec,uc] = mle(xc,'pdf',@(x,b)1/(2*b)*exp(-abs(x)/b),'Start',[.1]);
cec = heaviside(xc)-.5.*sign(xc).*exp(-abs(xc)/ec(1)); y = linspace(0,1,length(xc)); vc = max(cec-y) + max(y-cec);
...
for i = 1:nb; % generate probability distribution of maxima for Cenozoic
    xo = exprnd(eo+randn(1).*uo,1,round(6.6e7/(w.*1e6)));
    xc = exprnd(ec+randn(1).*uc,1,round(6.6e7/(w.*1e6)));
    Omax(i) = max(xo); Cmax(i) = max(xc); clear xo xc;
end
clear i w;
...
for i = 1:nb; % fit generated data with generating distribution
    [y,x] = ecdf(xo(i,:)); yy = 1-exp(-x./eo); vob(i) = max(y-yy) + max(yy-y);
end
xc = exprnd(ec,nb,length(ca)); % same for d13C
for i = 1:nb;
    [y,x] = ecdf(xc(i,:)); yy = 1-exp(-x./ec); vcb(i) = max(y-yy) + max(yy-y);
end
% output some results:
...
x(1,1:length(c)) = 0; % dummy variable -- delta prime
...
%b = mle(abs(x),'Distribution','exponential'); % estimate b 
b = mle(x,'pdf',@(x,b)1/(2*b)*exp(-abs(x)/b),'Start',[.1])

[y,xx] = ecdf(x); % find Kuiper statistic
yy(xx<0) = .5*exp(xx(xx<0)./b);
yy(xx>0) = 1-.5.*exp(-xx(xx>0)./b);
v = max(abs(y-yy')) + max(abs(yy'-y));
[v b] % Kuiper statistic and b parameter value of fit
...
p4 = scatter(htx,hty,100,[.6 .05 .4],'s','filled');
hold on;
p3 = scatter(hcx,hcy,100,[0.05 .5 .5],'filled');
p2 = scatter(hox,hoy,100,[0.7 0.4 0.05],'^','filled');
x = -.5:.001:.5;
l = .0708.*exp(-abs(x)./.0708); l = 29.*l./sum(l);
p1 = plot(x,l,'k','linewidth',2);
clear hoy hox hcx hcy x l;
axis([-.4 .4 0 .22])
box on;
axis square;
...
clear hoy hox hcx hcy;
axis([min(Omax-.05) max(Omax+.05) 0 .05])
box on;
axis square;
lgnd = legend([p1 p2],'\delta^{18}O (PETM \sim 1.7)','\delta^{13}C (PETM \sim 2.4)');
